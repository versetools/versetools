...
import type { GenericCtx } from "$convex/app/dataModel";
...
export default class ActionCache<Action extends FunctionReference<"action", FunctionVisibility>> {
	/**
	 * The name of the action cache. The name is part of the cache key and can be
	 * used for versioning. Defaults to the name of the action.
	 */
	...
	static async get(
		...
		if (match.metadataId) {
			const metadataDoc = await ctx.db.get("cachedActionMetadata", match.metadataId);
	...
	static async put(
		...
		if (args.ttl !== null) {
			...
			await ctx.db.patch("cachedActionValues", valueId, {
				metadataId
			});
		}
	...
	static async del(ctx: MutationCtx, value: Doc<"cachedActionValues">) {
		if (value.metadataId) {
			await ctx.db.delete("cachedActionMetadata", value.metadataId);
		}
		await ctx.db.delete("cachedActionValues", value._id);
	}
}
