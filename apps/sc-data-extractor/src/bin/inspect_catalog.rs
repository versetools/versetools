...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  for entry in p4k.entries() {
    ...
    {
      let family = entry.name.split('\\').take(4).collect::<Vec<_>>().join("/");
    ...
    .filter(|entry| is_datacore_path(&entry.name))
  {
    ...
    for record in db.records() {
      ...
      let record_path = db
        .resolve_string(record.file_name_offset)
        .replace('\\', "/");
      let fields = db
