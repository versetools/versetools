...
<div class="flex flex-col select-none">
	{#each Array.from( { length: 24 } ).flatMap( (_, h) => Array.from( { length: 2 } ).map((_, i) => new Time(h % 24, i * 30, 0)) ) as time, i (time)}
		<div class="relative h-8">
			{#if i % 2 === 0}
				<span
					class="text-text-40 absolute top-0 right-2 text-xs font-medium whitespace-nowrap uppercase"
				>
					{timeFormatter.format(new Date(0, 0, 0, time.hour))}
				</span>
			{:else if i === 24 * 2 - 1}
				<span
					class="text-text-40 absolute right-2 bottom-0 translate-y-1/2 text-xs font-medium whitespace-nowrap uppercase"
				>
