...
<script lang="ts">
  import { setContext, tick, type Component, type Snippet } from 'svelte'
  ...
  let tabContainer = $state<HTMLDivElement | undefined>()
  ...
  function computedItems() {
    ...
    itemWidthArray = (Array.from(tabContainer.querySelectorAll('.tab-header-item')) as HTMLElement[]).map(
      item => ({
