<script lang="ts">
  ...
  const activeIdx = $derived($pages.findIndex(p => !!p.to && !!routeId && isLinkActive(p.to, routeId)))
...
<div class="page-switcher">
  <div class:switcher={hasPrevPage}>
    {#if hasPrevPage}
      ...
      <a href={prevPage.to ? getPathFromBase(prevPage.to) : "#"} class="trigger">
        <div class="hint">
          {themeOptions.i18n?.previousPage || DEFAULT_PREVIOUS_TEXT}
        </div>
        <div class="title">
  ...
  <div class="right" class:switcher={hasNextPage}>
    {#if hasNextPage}
      ...
      <a href={nextPage.to ? getPathFromBase(nextPage.to) : "#"} class="trigger">
        <div class="hint">
          {themeOptions.i18n?.nextPage || DEFAULT_NEXT_TEXT}
        </div>
        <div class="title">
          <div class="title-label">
            {nextPage.title}
