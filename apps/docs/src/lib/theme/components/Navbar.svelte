<script lang="ts">
	...
	let DocsearchComponent = $state<
		Component<Omit<DocSearchProps, "theme" | "container">> | undefined
	>();
...
<header class="header" class:hidden-in-mobile={$scrollDirection === "down"} use:headerHeight>
	<div class="header-inner">
		...
		{#if SearchComponent || (themeOptions.search && typeof themeOptions.search !== "string")}
			<div class:is-home={isHome} class:move={!isHome && !hasError} class="doc-search">
				{#if SearchComponent}
					<SearchComponent />
				{:else if typeof themeOptions.search !== "string"}
					<themeOptions.search />
				{/if}
			</div>
		...
		<nav class="nav-links" aria-label="Menu">
			<div class="navbar-pc">
				<div class="none sm:flex">
					{#each themeOptions.navbar as navItem (navItem)}
						<NavItem {...navItem} />
					{/each}
				</div>
