...
<script lang="ts">
  ...
  import { tocCollapsed } from './layout'
	import type { Anchor } from '../markdown/anchors';

  const { anchors = [] }: TocProps = $props()
  ...
  onMount(() => {
    ...
    try {
      const ele = document.querySelector(anchorTarget) as HTMLElement
...
<Backdrop show={!$tocCollapsed} onClose={handleTocToggleClick} />
