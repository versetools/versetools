...
function getIconSafelist(themeOptions?: ThemeOptions): string[] {
...
export default async (options?: ThemeOptions) => {
	...
	const vitePluginsPre: PluginOption = [
		...
		}) as PluginOption,
		{
			...
			configureServer(server) {
				if (!autoSidebarRoutesDir || !isAutoSidebarOptions(options?.sidebar)) return;
				...
				server.watcher.on("add", (path) => {
					if (path.includes("+page.")) regenerate();
				});
