...
function isHeading(node: any) {
	...
	if (node.type === "html" && node.value.startsWith("<h")) {
		const match = node.value.match(/^<h([1-6])/);
...
function resolveTitleAndId(node: any) {
	let title = "";
	let slugId = "";
	if (node.type === "heading") {
		title = (node.children as any[])
		...
		if ("properties" in element && element.properties.id) {
			slugId = String(element.properties.id);
		}
	}
...
const anchors: Plugin<any[], any> = () => {
	return (tree, vFile) => {
		...
		visit(tree, (node, idx, parent) => {
			if (isHeading(node) && !node.data?.anchorAdded) {
				if (!node.data) {
					node.data = {
						anchorAdded: true
					};
				...
				let { title, slugId } = resolveTitleAndId(node);
