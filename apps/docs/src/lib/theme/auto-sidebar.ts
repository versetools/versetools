	...
	for (const name of subDirs) {
		...
		if (isGroupDir(name)) {
			...
			items.push(
				...groupItems.map((item) => ({
					...item,
					_order: (item as LinkItem & { _order: number })._order ?? 100
				}))
			);
			continue;
		}
		...
		const hasGroupChildren = childDirs.some(
			(n) => isGroupDir(n) && getSubDirs(join(subDirPath, n)).length > 0
		);
		const hasRealChildren = childDirs.some((n) => !isGroupDir(n));
		const hasChildren = hasRealChildren || hasGroupChildren;

		if (!pageFile && !hasChildren) continue;

		const meta = pageFile
			? getPageMeta(pageFile)
			: { title: "", order: 100, sidebar: true, collapsible: undefined };
		...
		if (hasChildren) {
			...
			if (meta.collapsible !== undefined) item.collapsible = meta.collapsible;
...
function detectRoots(routesDir: string): string[] {
	...
	for (const name of dirs) {
		if (shouldSkipDir(name) || isPrivate(name)) continue;
...
function hasPageFiles(dirPath: string): boolean {
	if (findPageFile(dirPath)) return true;

	for (const name of getSubDirs(dirPath)) {
		if (hasPageFiles(join(dirPath, name))) return true;
	}
	return false;
}
...
function findRootDir(routesDir: string, rootPath: string): string | null {
	...
	try {
		if (statSync(directPath).isDirectory()) return directPath;
	} catch {
		//
	}
	...
	for (const name of getSubDirs(routesDir)) {
		if (isGroupDir(name)) {
			...
			try {
				if (statSync(groupPath).isDirectory()) return groupPath;
			} catch {
				//
			}
		}
...
export function generateSidebar(autoOptions: AutoSidebarOptions): Record<string, LinkItem[]> {
	...
	for (const root of roots) {
		...
		if (items.length > 0) sidebar[root] = items;
	}
